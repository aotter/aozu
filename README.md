...
`@aotter/mantle-web` projects each public Mantle capability to the browser's
imperative WebMCP API. The binding is equivalent to the following registration;
AOZU keeps the production implementation centralized in
[`src/adapters/webmcp/tools.ts`](./src/adapters/webmcp/tools.ts):
...
document.modelContext.registerTool({
  ...
  execute: (input) => invokeMantle(capability, input),
})
```
