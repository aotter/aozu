...
import { commitItemAction, planItemEffects } from './core/application/items.ts'
...
export function createApplication(document: Document) {
  ...
  const application = {
    async loadStartup() {
      ...
      return {
        ...startup,
        loadout: (await planItemEffects(entries, startup.runId, [])).projection,
    ...
    async setItemState(inventoryId: string, state: Record<string, unknown>, expectedRevision: number, idempotencyKey: string = crypto.randomUUID()) {
      const { bundleId, runId } = await active()
      return commitItemAction(createIndexedDbEntryRepository(bundleId), createIndexedDbActionRepository(), {
        bundleId,
        runId,
        expectedRevision,
        idempotencyKey,
        effects: [{ type: 'setItemState', inventoryId, state }],
      })
    },
