...
export interface AgentCustomization {
  ...
  items?: Array<{
    ...
    stateSchema?: ItemDefinition['stateSchema']
  }>
...
export function assembleAuthoredCandidate(
  ...
  const entries = [
    ...
    ...(customization.items ?? []).flatMap(({ inventoryId, state = {}, ...definition }) => [
      {
        id: `definition:${definition.id}`,
        collection: 'item-definitions',
        data: { definition: { ...definition, stackable: false } },
      },
      {
        id: inventoryId,
        collection: 'inventory-items',
        data: { definitionId: definition.id, quantity: 1, state },
      },
    ]),
    ...((customization.items?.length ?? 0) > 0 ? [{
      id: `loadout:${runId}`,
      collection: 'character-loadouts',
      data: { runId, equipment: customization.initialEquipment ?? {}, appearanceOverrides: {} },
    }] : []),
  ]
...
function validateCustomization(customization: AgentCustomization) {
  ...
  for (const item of customization.items ?? []) {
    if (!item.id || !item.name || !item.inventoryId || !item.equipSlot || itemIds.has(item.id) || inventoryIds.has(item.inventoryId)) {
      throw new Error('Invalid or duplicate customization item')
    }
    itemIds.add(item.id)
    inventoryIds.add(item.inventoryId)
  }
  for (const inventoryId of Object.values(customization.initialEquipment ?? {})) {
    if (inventoryId && !inventoryIds.has(inventoryId)) throw new Error(`Unknown initial equipment: ${inventoryId}`)
  }
