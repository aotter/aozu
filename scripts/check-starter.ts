...
assert.equal(packages[0]!.starter.directions[0]!.playbook.initialStageId, 'study-session')
assert.equal(packages[0]!.starter.characterStates[0]!.name, 'Focus Friend')
...
assert.equal(draft.story!.starter.manifestSha256, packages[0]!.manifestSha256)
assert.equal('assets' in draft, false)
assert.equal('stages' in draft, false)
assert.equal('playbook' in draft.story!.direction, false)
const character = createCharacterDraftFromStarter(packages[0]!, 'character:focus-default')
...
assert.deepEqual(resolveStarterCharacterLayers(packages[0]!, 'character:focus-default').map(({ slot }) => slot), ['character-skin', 'expression-head'])
