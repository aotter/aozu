...
const ALL_BACKBONE_SOURCES = [
  ...
  source(
    ...
    envelope('Procedure', 'inspect-character-contract', {
      ...
      description: 'Required before generating or changing character art. Optionally name one target to receive its exact reference layer, deterministic editable-region mask, alpha-mask diagnostics, z-order, canonical freshness, and alignment mode. Mask transparency marks editable pixels; opaque pixels are protected. After submission, protectedRegionDelta reports non-blocking protected-pixel drift; 0 means unchanged and lower is better. A registered head anchor may include an experimental native pixel-and-edge correlation suggestion; it is never applied or approved automatically. Outfits are complete dressed character-skin replacements, not clothing-only overlays. Props have no editable-region mask. The website validates but never removes backgrounds, resizes, realigns, or repairs images.',
      input: {
        ...objectSchema({
          draftId: { type: 'string', minLength: 1 },
