...
const suggestedTransform = (reference: CharacterAlphaMask, candidate: CharacterAlphaMask): CharacterVariantTransform | null => {
  ...
  return {
    scale,
    x: expected.x - actual.x * scale,
    y: expected.y - actual.y * scale,
  }
}
...
export function measureCharacterMaskAlignment(
  ...
  if (group === 'prop' || group === 'body' || !reference) {
    return {
  ...
  const aligned = current.iou >= 0.85 && centered && (group !== 'outfit' || Math.abs(current.footLineDelta ?? Infinity) <= 12)
  return {
